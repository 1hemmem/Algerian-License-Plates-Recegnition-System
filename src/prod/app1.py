...
def ocr_thread(ocr, completed_tracks, sharpness, lock):
    while True:
        with lock:
            for track_id, track_data in completed_tracks.items():
                if not track_data["ocred"]:
                    plate_path = track_data["plate_path"]
                    try:
                        plate_number = ocr.extract_text_from_image(image_path=plate_path)
                        track_data["plate_number"] = plate_number
                        track_data["ocred"] = True
                        ...
                        logger.error(f"Error extracting text from {plate_path}: {e}")
...
def main():
    ...
    # Lock for thread-safe access to completed_tracks
    lock = threading.Lock()

    # Start the OCR thread
    ocr_thread_instance = threading.Thread(target=ocr_thread, args=(ocr, completed_tracks, sharpness, lock))
    ocr_thread_instance.daemon = True
        ...
        while True:
            ...
            if detections:
                ...
                for track in tracks:
                    ...
                    with lock:
                        if track_id in completed_tracks and not completed_tracks[track_id]["ocred"]:
                            continue
